...
packaged as a single portable `.sqlite` file. No backend, no install: open the
...
One `.sqlite` file = one catalog ("semantic set"). Tables:
...
add rows, export/reopen the `.sqlite`, view + highlight-on-click), but UI and
